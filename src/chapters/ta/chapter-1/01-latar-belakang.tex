...
% FHIR adalah standar dan regulasi pertukaran data rekam medis yang sudah diadopsi oleh World Health Organization (WHO) \autocite{fhirspec}.
% Meskipun FHIR adalah standar internasional yang agnostik terhadap arsitektur (bisa digunakan secara sentral maupun terdistribusi), implementasi di Indonesia sangat bergantung pada server FHIR terpusat \autocite{fhirsatusehat}.
...
Ketika suatu modul atau komponen sistem mengalami kegagalan, maka seluruh sistem dapat mengalami kegagalan total \autocite{jin2020blockchainIPFSmedicalrecord}.
...
Berbagai implementasi sistem rekam medis yang terdesentralisasi sudah dibuat untuk mengatasi masalah-masalah di atas.
Implementasi seperti blockchain, Ethereum, dan IOTA masif digunakan untuk menciptakan sistem rekam medis yang terdesentralisasi \autocite{zhengdltcomparisonmedicalrecord}.
Dari berbagai teknologi desentralisasi yang disebutkan, IOTA merupakan teknologi desentralisasi yang paling efisien karena tidak membutuhkan biaya (\textit{gas fee}) dan \textit{resources} yang tinggi dan juga terdesentralisasi penuh \autocite{zhengdltcomparisonmedicalrecord}.
...
Penggunaan IOTA memberikan penyelesaian permasalahan sistem rekam medis elektronik yang tersentralisasi sekaligus menghindari implementasi berbasis \textit{blockcchain} yang memiliki komputasi mahal.
...
\begin{itemize}
    \item Pasien harus memberikan persetujuan akses data secara langsung (\textit{synchronous}) dengan melakukan \textit{scan} suatu QR code yang diberikan oleh fasyankes untuk memberikan izin akses terhadap datanya di fasyankes. 
    Hal ini membuat pasien harus hadir secara langsung untuk memberikan akses dan pasien tidak dapat memberikan izin kepada pihak lain saat pasien tidak sadar atau tidak cakap untuk menggunakan perangkatnya.
...
Berdasarakan dua permasalahan ini, Tugas Akhir mengusulkan penggunaan \textit{authorization framework} dan \textit{portal consent} bagi pasien dalam sistem rekam medis DecMed.
...
Selain itu, implementasi \textit{portal consent} bagi pasien juga dilakukan untuk memberikan kontrol yang lebih granular dan keleluasaan dalam penggunaannya bagi pasien \autocite{dara2020patientconsent}.
