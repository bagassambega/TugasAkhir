...
Berdasarkan analisis masalah yang sudah dijabarkan, diperlukan modifikasi terhadap sistem DecMed: (\textbf{1}) agar DecMed mampu memberikan granularitas pengelolaan akses terhadap data pasien, dan (\textbf{2}) agar DecMed mampu menyediakan pemberian akses secara asinkronus.


% Solusi yang diusulkan untuk mengatasi keterbatasan sistem DecMed ini adalah pengimplementasian \textit{authorization framework} dan pembuatan \textit{consent portal} untuk pasien.
...
% Sementara itu \textit{consent portal} dibuat untuk memberikan informasi dan platform bagi pasien untuk mengelola data dan akses secara granular dan informatif.

Untuk mengatasi limitasi tersebut, solusi yang diajukan adalah integrasi Authorization Framework yang dipadukan dengan Patient Consent Portal. 
Authorization Framework berfungsi sebagai Policy Decision Point (PDP) sehingga pasien membuat keputusan akses berdasarkan aturan bukan sekadar memeriksa daftar statis. 
Dalam konteks kasus masalah asinkronus, kerangka kerja ini memungkinkan pasien untuk mendefinisikan kebijakan akses pre-emptive di masa lalu.

Berdasarkan kajian yang sudah dilakukan pada subbab \@\ref{subsec:authorization-framework-2}, pemilihan protokol yang tepat untuk Authorization Framework ini memerlukan komparasi antara SAML 2.0 (Security Assertion Markup Language), OAuth 2.0, dan UMA (User-Managed Access) 2.0. 
SAML kuat dalam federasi identitas (Single Sign-On) namun sangat bergantung pada pertukaran XML yang berat dan umumnya didesain untuk otentikasi sinkronus antar organisasi, bukan untuk delegasi akses sumber daya spesifik antar individu. 
OAuth 2.0 merupakan standar industri untuk delegasi, namun sangat bergantung pada persetujuan pengguna secara langsung (interactive consent) pada saat akses diminta, yang gagal memenuhi kebutuhan kasus gawat darurat (asinkronus). Selain itu, OAuth standar tidak memiliki mekanisme terpusat bagi Resource Owner untuk mengelola kebijakan akses setelah token diterbitkan.

Oleh karena itu, UMA 2.0 dipilih sebagai solusi arsitektural. UMA adalah profil lanjutan dari OAuth 2.0 yang secara spesifik dirancang untuk memisahkan proses otorisasi dari pemilik sumber daya.
UMA 2.0 memungkinkan pasien (pemilik sumber daya) untuk mendefinisikan policy atau aturan akses kontrol secara asinkronus, sehingga jika data pasien tersebut dibutuhkan, pihak peminta akses data (requesting party) dapat meminta langsung data ke authorization server.
...
\begin{table}[H]
	...
	\begin{tabular}{|p{2.5cm}|p{2.8cm}|p{2.8cm}|p{2.8cm}|} % chktex 44
		\hline % chktex 44
			\textbf{Fitur / Kriteria} & \textbf{SAML 2.0} & \textbf{OAuth 2.0} & \textbf{UMA 2.0} \\
		\hline % chktex 44
		Tujuan/Desain Utama & SSO, federasi & User ke aplikasi & User ke pihak lain \\
		\hline % chktex 44
		Model Persetujuan & Federasi institusi & User ke app & User ke individu \\
		\hline % chktex 44
		Granularitas Akses & Rendah & Sedang & Sangat tinggi \\
		\hline % chktex 44
		Format Data & XML & JSON/JWT & JSON/JWT \\
		\hline % chktex 44
		Policy Asinkronus & Tidak ada & Tidak baku & Ada \\
		\hline % chktex 44
		Fleksibilitas Kebijakan & Kaku & Statis & Dinamis \\
		\hline % chktex 44
	\end{tabular}
\end{table}
...
% Selain itu, sebagai ekstensi dari OAuth 2.0, UMA juga memenuhi standar interoperabilitas FHIR yang 
...
Consent portal pasien akan dibentuk sebagai client terpisah dan berperan sebagai antarmuka pendefinisian policy (PAP, Policy Administration Point).
Consent portal berisi pendefinisian policy/akses kontrol yang diberikan oleh pasien.
Pasien dapat mendefinisikan data apa, kepada siapa, tindakan apa yang bisa dilakukan, dan sampai kapan akses tersebut diberikan.
% Consent portal pasien dapat diakses setelah pasien terautentikasi menggunakan mekanisme autentikasi OpenID Connect (OIDC) 2.0 yang berjalan di atas UMA2.0.
...
Sequence diagram DecMed yang mengimplementasikan authorization framework dan consent portal bagi pasien dapat dilihat pada Lampiran~\ref{fig:sequence-diagram}.
