...
Kesenjangan informasi ini menjadi hambatan bagi tenaga kesehatan dalam memperoleh gambaran kondisi pasien yang lebih lengkap pada saat melakukan evaluasi klinis misalnya untuk kondisi yang memerlukan pemantauan berkala.
...
Kedua, arsitektur DecMed belum mengimplementasikan mekanisme \textit{offline-first} pada klien pasien dan belum menyediakan penyimpanan lokal serta sinkronisasi bertahap untuk data kesehatan yang dihasilkan di luar konteks kunjungan klinis. 
...
Kebutuhan akan pencatatan asal-usul, riwayat pemrosesan, dan rantai kepemilikan data sejak pertama kali dikumpulkan menjadi hal yang penting dalam bagi PGHD\@ \parencite{Ahmed_2023_Data_Provenance}. 
...
\begin{table}[htbp]
    ...
    \begin{styledtable}{>{\raggedright\arraybackslash}p{0.10\textwidth} >{\raggedright\arraybackslash}X}
        \textbf{Kode Gap} & \textbf{Deskripsi Permasalahan} \\
        
        Gap 1 & Tidak adanya jalur pemasukan (\textit{ingest}) PGHD dari sisi pasien; seluruh entri data pada DecMed saat ini hanya berasal dari tenaga kesehatan melalui kunjungan klinis. \\
...
\subsection{Analisis Kerahasiaan, Integritas dan Ketersediaan DecMed - PGHD}
...
\begin{table}[htbp]
    ...
    \begin{styledtable}{>{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.15\textwidth} >{\raggedright\arraybackslash}p{0.08\textwidth} >{\raggedright\arraybackslash}X}
        ...
        \rowcolor{rowOdd}
        \Block{2-1}{\textit{Tampering}} & \Block{2-1}{\centering Integritas} & \texttt{V-T01} & \textit{Data flow} melalui jaringan dapat dirusak oleh penyerang. \\
        \rowcolor{rowOdd}
        ...
        \rowcolor{rowEven}
        \Block{3-1}{\textit{Information Disclosure}} & \Block{3-1}{\centering Kerahasiaan} & \texttt{V-I01} & Perlindungan data yang tidak tepat pada \textit{server storage} dapat memungkinkan penyerang membaca informasi yang hanya bisa diakses oleh \textit{authorized party}. \\
        \rowcolor{rowEven}
        ...
        \rowcolor{rowOdd}
        \Block{2-1}{\textit{Denial of Service}} & \Block{2-1}{\centering Ketersediaan} & \texttt{V-D01} & \textit{Mobile app} dapat berhenti, berjalan lambat, atau melanggar metrik ketersediaan. \\
        \rowcolor{rowOdd}
