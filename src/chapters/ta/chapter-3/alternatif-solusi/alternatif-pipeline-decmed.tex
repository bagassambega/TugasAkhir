...
\subsubsection{Sumber Data PGHD}\label{alternatif-sumber-data}
...
\begin{enumerate}
    ...
    \item Perangkat \textit{mobile} dan aplikasi,
...
\begin{enumerate}
  \item Perangkat sensor dan \textit{wearable},
  \item Perangkat \textit{mobile} dan aplikasi, dan
  \item Laporan mandiri pasien.
\end{enumerate}

Perangkat medis, perangkat pemantauan dan sensor rumahan maupun lingkungan tidak dipertimbangkan karena memerlukan mekanisme validasi medis.
Data eksternal seperti data yang berasal dari media sosial tidak dipertimbangkan karena memerlukan mekanisme tambahan untuk memfilter data yang merupakan data PGHD ataupun bukan.
...
\subsubsection{Pengumpulan Data PGHD}\label{alternatif-pengumpulan-pghd}
...
Pada integrasi DecMed dan PGHD ini, mekanisme yang dipilih adalah kombinasi alternatif pertama dan kedua, yaitu mengumpulkan dan menyimpan data \textit{raw} terlebih dahulu di penyimpanan lokal \textit{smartphone}, kemudian dibuatkan agregasi data sebagai visualisasi data yang akan ditampilkan kepada tenaga kesehatan.
Jadi data asli akan dipertahankan dan agregasi data akan tetap dibuat, sehingga tidak ada detail informasi yang hilang dari data PGHD yang dikumpulkan, namun tenaga kesehatan dapat mendapatkan informasi kritis dan penting dari PGHD yang dikirim juga.

\subsubsection{Pengolahan Data PGHD}\label{alternatif-pengolahan-pghd}

Pengolahan data dilakukan agar tenaga kesehatan dapat mendapatkan informasi penting dari PGHD\@.
Terdapat tiga alternatif mekanisme yang dipertimbangkan.

\begin{enumerate}
    \item \textbf{Tampilan daftar entri PGHD tanpa reduksi.} 
    Seluruh entri \textit{batch} ditampilkan kepada tenaga kesehatan dalam bentuk daftar kronologis tanpa mekanisme prioritisasi atau pengolahan tambahan. 
    Tenaga kesehatan harus membuka setiap entri secara manual untuk mengetahui isinya. 
    Alternatif ini tidak menyediakan mekanisme apapun untuk membantu tenaga kesehatan mengidentifikasi entri yang bermakna klinis, sehingga beban penelusuran data sepenuhnya dibebankan kepada tenaga kesehatan.

    \item \textbf{Filter anomali sebelum pengiriman batch.} 
    Klien pasien melakukan deteksi anomali berbasis \textit{threshold} klinis sebelum pembentukan \textit{batch} dan hanya \textit{record} yang terdeteksi anomali yang dikemas ke dalam \textit{batch}. 
    Data normal disimpan secara lokal tetapi tidak dikirimkan ke DecMed. 
    Kelemahan alternatif ini adalah tenaga kesehatan tidak dapat membedakan antara \textit{batch} yang tidak terkirim karena tidak ada anomali dengan \textit{batch} yang tidak terkirim karena kegagalan teknis, sehingga menyulitkan interpretasi klinis. 
    Selain itu, deteksi anomali memiliki risiko \textit{false positive}.

    \item \textbf{Agregasi statistik dengan penanda anomali berbasis \textit{threshold}.} 
    Seluruh data PGHD tetap dikirim dan disimpan di sistem DecMed. 
    Pada saat pembentukan \textit{batch}, sebelum enkripsi dilakukan, klien pasien menghitung ringkasan statistik agregat, yaitu nilai minimum, maksimum, persentil, modus, median dan rata-rata untuk setiap tipe data dalam \textit{batch}, serta membandingkan nilai-nilai tersebut dengan \textit{threshold} klinis yang telah dikonfigurasi. 
    Tipe data yang memiliki nilai di luar rentang \textit{threshold} dicatat sebagai penanda anomali. 
    Di klien tenaga kesehatan, klien akan membangun grafik untuk menunjukkan tren data pasien.
\end{enumerate}

Pada integrasi DecMed dan PGHD ini, mekanisme yang dipilih adalah alternatif ketiga. 
Ringkasan statistik dan penanda anomali ditampilkan langsung kepada tenaga kesehatan, tapi data asli tetap tersimpan utuh dan dapat dilihat secara detail oleh tenaga kesehatan. 

\subsubsection{Pengiriman Data PGHD}\label{alternatif-trigger-pengiriman-pghd}
...
\textit{Query-driven} tidak dipertimbangkan karena justru menjadi beban bagi pasien karena permintaan data tidak terjadwal atau tidak dapat diprediksi oleh pasien dan dapat menyebabkan volume/frekuensi pengiriman data tidak terprediksi.

Pada integrasi DecMed dan PGHD, mekanisme yang dipilih adalah kombinasi antara pengiriman periodik dan \textit{event-driven}. 
Pengiriman dilakukan secara periodik sehingga akan terbentuk \textit{batch}, sehingga pola pengiriman tetap stabil dan deret waktu PGHD dapat dibentuk dengan baik.
...
Pada integrasi DecMed dan PGHD, mekanisme yang dipilih adalah akses berbasis kunjungan yang juga menerapkan akses \textit{on-demand} ketika tenaga kesehatan meminta izin untuk mengakses data pasien.
...
Pendekatan ini sesuai dengan pola pemanfaatan PGHD sebagai data pelengkap yang digunakan untuk mendukung keputusan klinis dalam konteks konsultasi, bukan sebagai aliran data yang harus dimonitor terus-menerus, karena akses data PGHD secara terus menerus juga dapat memecah fokus tenaga kesehatan.
