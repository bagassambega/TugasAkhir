...
\begin{enumerate}
    ...
    Setelah struktur data gabungan (plaintext + \textit{ inner signature}) berhasil dienkripsi menjadi \verb|enc_pghd|, klien pasien menyimpan batch PGHD ke dalam \textit{local encrypted database} dengan struktur format data yang dijelaskan pada subbab~\ref{rancangan-solusi-format-data-lokal}.
    \textit{Local database} dienkripsi menggunakan AES-GCM dengan kunci yang diturunkan dari PIN pasien menggunakan PBKDF2 (iterations: 100.000, output length: 256 bit) dan disimpan di \textit{secure storage} perangkat. 
    Enkripsi ini memastikan bahwa data PGHD yang tersimpan secara lokal tetap aman meskipun perangkat hilang atau dicuri. 
    ...
    \item\label{fungsionalitas-retry-mechanism} \textbf{Mekanisme \textit{retry} dan sinkronisasi \textit{offline}}. 
    Klien pasien menjalankan \textit{background service} yang secara periodik (setiap 5 menit) melakukan langkah-langkah berikut untuk batch yang gagal dikirim:
    ...
    \begin{enumerate}
        ...
        \item Jika pengiriman batch gagal (misalnya karena timeout, proxy error, atau error jaringan), klien memperbarui:
...
Mekanisme retry eksponensial diterapkan dengan interval: percobaan pertama setelah 5 menit, kedua setelah 15 menit, ketiga setelah 30 menit. Pemilihan interval ini didasarkan pada keseimbangan antara peluang pemulihan koneksi dan konsumsi baterai perangkat.
