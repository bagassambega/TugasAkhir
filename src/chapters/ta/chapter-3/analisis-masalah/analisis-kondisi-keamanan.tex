...
Analisis pada subbab ini bertujuan menentukan persoalan keamanan dan properti yang harus dipenuhi oleh integrasi PGHD, yaitu kerahasiaan, integritas, dan ketersediaan.
Analisis mengikuti tahapan \textit{threat modelling} pada Subbab~\ref{threat-model-studi}, yaitu mengidentifikasi aset dan ruang lingkup, menentukan aktor dan batas kepercayaan, dan mengidentifikasi ancaman menggunakan STRIDE\@.
Ruang lingkup analisis mencakup PGHD sejak tersedia pada sisi pasien hingga digunakan oleh tenaga kesehatan.
Tahapan yang dipertimbangkan hanya meliputi perolehan, pengolahan, pengiriman, penyimpanan, dan penggunaan data.
...
\begingroup
\footnotesize
\begin{styledxltabular}{|>{\raggedright\arraybackslash}p{0.10\textwidth}|>{\raggedright\arraybackslash}p{0.27\textwidth}|>{\raggedright\arraybackslash}X|}
    \hiderowcolors
    ...
    \hiderowcolors
    \caption[]{Aset pada Integrasi DecMed-PGHD (lanjutan)}\\
    \hline
    \rowcolor{headerBg}
    \textbf{ID} & \textbf{Aset} & \textbf{Alasan Perlindungan} \\
    \hline
    ...
    \hline
    A02 & Metadata PGHD dan \textit{provenance} & Menjelaskan pemilik, waktu, sumber, cara perolehan, dan riwayat pemrosesan data sehingga memengaruhi interpretasi serta tingkat kepercayaan terhadap PGHD\@. \\
    \hline
...
DFD Level 1 pada Gambar~\ref{fig:dfd-decmed-pghd-level-1} menguraikan proses pengumpulan dan pembentukan \textit{batch}, autentikasi dan kontrol akses, penyimpanan dan pengambilan PGHD, verifikasi dan pengaksesan PGHD\@.

\clearpage
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,height=0.48\textheight,keepaspectratio]{resources/chapter-3/Diagram Tugas Akhir-DFD Level 1.png}
    \caption{DFD Level 1 Sistem DecMed-PGHD untuk \textit{Threat Modelling}}\label{fig:dfd-decmed-pghd-level-1}
\end{figure}

Tata letak DFD dirapatkan tanpa mengubah hubungan antarkomponen agar identitas proses, penyimpanan, batas kepercayaan, dan label aliran F1--F15 tetap terbaca.
...
\begingroup
\footnotesize
\begin{styledxltabular}{|>{\raggedright\arraybackslash}p{0.08\textwidth}|>{\raggedright\arraybackslash}p{0.18\textwidth}|>{\raggedright\arraybackslash}p{0.28\textwidth}|>{\raggedright\arraybackslash}X|}
    \hiderowcolors
    ...
    \hiderowcolors
    \caption[]{Analisis \textit{Trust Boundary} DecMed-PGHD (lanjutan)}\\
    \hline
    \rowcolor{headerBg}
    \textbf{ID} & \textbf{Zona} & \textbf{Komponen} & \textbf{Risiko Utama} \\
    \hline
    ...
    \hline
    TB3 & \textit{DLT zone} & Jaringan IOTA, \textit{smart contract public state}, transaksi \textit{on-chain} & Metadata terlihat pada \textit{state} jaringan, transaksi tidak dapat dihapus, serta biaya/gas dan ketersediaan transaksi bergantung pada konfigurasi node IOTA dan Gas Station. \\
    \hline
...
\begingroup
\footnotesize
\begin{styledxltabular}{|>{\raggedright\arraybackslash}p{0.20\textwidth}|>{\raggedright\arraybackslash}p{0.20\textwidth}|>{\raggedright\arraybackslash}X|}
    \hiderowcolors
    ...
    \hiderowcolors
    \caption[]{Aktor, \textit{\textit{Privilege}}, dan \textit{\textit{Trust Level}} (lanjutan)}\\
    \hline
    \rowcolor{headerBg}
    \textbf{Aktor} & \textbf{\textit{Trust Level}} & \textbf{\textit{Privilege} dan Batasan} \\
    \hline
...
Sesuai ruang lingkup analisis kerahasiaan, integritas, dan ketersediaan, kategori ancaman yang digunakan adalah \textit{Tampering}, \textit{Information Disclosure}, dan \textit{Denial of Service}.
