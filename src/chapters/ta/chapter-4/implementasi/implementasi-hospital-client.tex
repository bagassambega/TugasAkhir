...
% Diagram kelas pada \textit{client} tenaga kesehatan dapat dilihat pada Gambar~\ref{fig:diagram-modul-client-tenaga-kesehatan-pghd}.

% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=\textwidth]{resources/chapter-4/diagram-modul-client-tenaga-kesehatan-pghd.png}
%     \caption{Diagram Kelas \textit{Client} Tenaga Kesehatan PGHD}\label{fig:diagram-modul-client-tenaga-kesehatan-pghd}
% \end{figure}
...
Halaman PGHD digunakan untuk melihat daftar \textit{batch} PGHD pasien, membuka detail \textit{batch} PGHD, melihat \textit{provenance} data, ringkasan statistik, visualisasi, penanda anomali, dan menginvalidasi data PGHD apabila data terbukti tidak valid.
...
% \textit{Access token} tersebut digunakan untuk memanggil \textit{endpoint} daftar PGHD dan endpoint detail PGHD pada PRE, yang masing-masing dijelaskan pada Subbab~\ref{subsubsec:pre-endpoint-pghd-list} dan Subbab~\ref{subsubsec:pre-endpoint-pghd-detail}. 
...
% Pertama, \textit{client} membandingkan hash dari \textit{ciphertext} yang diunduh dengan hash yang tercatat pada metadata. 
...
% Keempat, \textit{client} melakukan dekripsi AES-GCM\@. 
...
% Setelah dekripsi berhasil, \textit{frontend} membaca struktur skema 1.1 dan membentuk tab berdasarkan \texttt{measurement\_type}. Setiap tab menampilkan jumlah anomali, kartu statistik untuk setiap \textit{field}, rentang klinis yang dibawa oleh \textit{payload} beserta konteks populasi dan tautan referensi, visualisasi, serta tabel data asli. Komponen \texttt{PghdMeasurementVisualization} memilih bentuk grafik berdasarkan karakteristik data. Deret waktu fisiologis seperti detak jantung, saturasi oksigen, laju napas, suhu, dan tekanan darah menggunakan grafik garis. Nilai periodik kumulatif seperti langkah, jarak, dan kalori menggunakan grafik batang. Tipe numerik lain dengan jumlah sampel yang memadai menggunakan \textit{box plot} untuk menunjukkan distribusi.

% Pada grafik garis, batas klinis digambar sebagai garis putus-putus dan data di luar rentang digambar sebagai titik merah. Pada grafik batang atau \textit{box plot}, elemen anomali juga diberi warna merah. Tabel detail di bawah grafik tetap memuat waktu, nilai asli, sumber, dan metode pencatatan; baris anomali diberi latar merah serta keterangan di bawah atau di atas rentang. Pesan pada antarmuka menegaskan bahwa penanda ini merupakan bantuan penyaringan, bukan diagnosis. Untuk kompatibilitas mundur, \textit{batch} skema lama yang tidak mempunyai \texttt{statistics} atau \texttt{clinical\_thresholds} tetap ditampilkan menggunakan data mentah yang berhasil didekripsi.

% Pemetaan struktur \textit{payload} ke antarmuka dapat dilihat pada Tabel~\ref{tab:implementasi-visualisasi-pghd-hospital}.

% \begin{styledxltabular}{|>{\raggedright\arraybackslash}p{0.29\textwidth}|>{\raggedright\arraybackslash}p{0.27\textwidth}|>{\raggedright\arraybackslash}X|}
%     \hiderowcolors
%     \caption{Implementasi Penyajian PGHD pada \textit{Client} Tenaga Kesehatan}\label{tab:implementasi-visualisasi-pghd-hospital} \\
%     \hline
%     \rowcolor{headerBg}
%     \textbf{Data} & \textbf{Komponen UI} & \textbf{Penyajian} \\
%     \hline
%     \showrowcolors
%     \endfirsthead
%     \hline
...
%     \hline
%     \endhead
%     \texttt{measurement\_type} & Tab tipe data & Memisahkan tiap tipe agar ringkasan dan detail tidak tercampur. \\
%     \hline
%     \texttt{statistics} & Kartu statistik & Menampilkan jumlah, minimum, maksimum, rata-rata, median, modus, dan P5--P95. \\
%     \hline
%     \texttt{clinical\_\allowbreak{}thresholds} & Kartu rentang klinis & Menampilkan interval, satuan, konteks populasi, dan referensi yang ikut dikirim pasien. \\
%     \hline
%     \texttt{data\_points} dan \texttt{anomalies} & Grafik adaptif & Grafik garis, batang, atau \textit{box plot}; anomali ditandai merah. \\
%     \hline
%     \texttt{data\_points} & Tabel detail & Menampilkan seluruh nilai asli dan \textit{provenance}; baris anomali diberi penanda. \\
%     \hline
% \end{styledxltabular}
...
% \textit{Medical record} tetap mengikuti mekanisme DecMed lama yang dibagi menjadi data administratif dan klinis. 
