...
\begin{styledxltabular}{|>{\raggedright\arraybackslash}p{0.10\textwidth}|>{\raggedright\arraybackslash}p{0.20\textwidth}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
    ...
    \hline
    T-NF02 & \textit{Integrity} (NF02) & Submit PGHD valid, lalu gunakan data uji yang ciphertext, hash, atau signature-nya diubah. Data tersebut kemudian dicoba dibuka melalui client tenaga kesehatan. & Sistem mendeteksi perubahan melalui hash atau digital signature, menolak pembukaan data, dan menandai metadata PGHD sebagai tidak valid. \\
    \hline
...
\begin{styledxltabular}{|>{\raggedright\arraybackslash}p{0.12\textwidth}|>{\raggedright\arraybackslash}p{0.20\textwidth}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}p{0.18\textwidth}|>{\raggedright\arraybackslash}p{0.10\textwidth}|}
    ...
    \hline
    T-NF02 & \textit{Integrity} & Data yang diubah terdeteksi dan tidak dapat dibuka sebagai data valid. & \texttt{ERR\_DATA\_CORRUPTED}, signature error, atau status \texttt{INVALID}. & Sukses \\
    \hline
...
Skenario T-NF02 dilakukan untuk menguji aspek \textit{integrity}. Sistem harus mampu mendeteksi apabila data PGHD rusak atau dimodifikasi setelah dikirim. Pengujian dilakukan dengan membandingkan data PGHD valid dengan data uji yang sudah diubah pada bagian ciphertext, hash, atau signature.

Prosedur yang dilakukan pada skenario ini adalah sebagai berikut.
\begin{enumerate}
    \item Pasien mengirimkan satu batch PGHD valid.
    \item Penguji membuat salinan payload atau fixture PGHD yang nilai ciphertext, hash, atau signature-nya diubah.
    \item Tenaga kesehatan mencoba membuka data PGHD yang sudah dimodifikasi.
    \item Sistem melakukan verifikasi hash dan digital signature.
    \item Jika data tidak valid, sistem menolak pembukaan data dan menandai metadata sebagai \texttt{INVALID}.
\end{enumerate}

Script pengujian integrity ditunjukkan pada Kode~\ref{lst:nf02-integrity}. Script ini menjalankan pengujian pada PRE untuk signature PGHD dan pengujian \textit{smart contract} untuk memastikan entri PGHD yang invalid tidak dapat dibuka.
